...
small_strtod(const char* str, char** endptr)
{
  ...
  if ((mant0|mant1|mant2) != 0) {
    const int MAX_EXP_MAGNITUDE = 352;
    rdVal0 = (rdVal0 >> 4) | (rdVal1 << 24);
    rdVal1 >>= 8;
    if ((rdVal2 | rdVal1) != 0)
      rdVal0 = UINT32_MAX;
    int64_t exp64 = nege==0 ? (int64_t)rdVal0 : -(int64_t)rdVal0;
// printf("%I64d+%d\n", exp64, (int)exp);
    exp64 += exp;
...
// printf("%08x:%08x:%08x %u.%u\n", mant2, mant1, mant0, sexp,mexp);
      uint32_t w2 = mant2;
